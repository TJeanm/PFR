...
void choix_langue()
{
  ...
  if (!fichier) {
    ...
    FILE *fichier_langue = fopen("commande_vocale/choix_langue.txt", "w");
    if (!fichier_langue) {
      perror("Erreur lors de l'ouverture du fichier choix_langue.txt");
    } else {
...
void commande_vocal()
{
  ...
  int distance;
  int action_detecte;
  char *command = "python3 commande_vocale/Vocal.py";
...
void simulation_vocal () {
    const char *command = "python3 commande_vocale/visualisation_turtle.py";
