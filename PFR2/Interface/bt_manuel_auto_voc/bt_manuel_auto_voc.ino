...
const int DISTANCE_SEUIL_ARRIERE=40;
...
void loop() {
  //Reception Bluetooth
   distanceFront1 = getDistance(FRONT_TRIGGER1, FRONT_ECHO1);
   distanceFront2 = getDistance(FRONT_TRIGGER2, FRONT_ECHO2);
   distanceRear  = getDistance(REAR_TRIGGER, REAR_ECHO);
  ...
  if (currentState=='i'){
      modeVocal(var,distanceFront1,distanceFront2);
  }
  if (currentState=='p'){
      modeManuel(var);
  }
...
void modeAuto(long distanceFront1,long distanceFront2){
  if (distanceFront1 > DISTANCE_SEUIL || distanceFront1 == 0 && distanceFront2 > DISTANCE_SEUIL || distanceFront2 == 0) {
    avancer(VITESSE);
    if (distanceFront1 < DISTANCE_SEUIL && distanceFront1 != 0 || distanceFront2 < DISTANCE_SEUIL && distanceFront2 != 0) {
      if(distanceRear>0 && distanceRear < DISTANCE_SEUIL_ARRIERE){
        droite();
      }else{
        reculer(VITESSE);
        delay(400);
...
void modeVocal(char val,long distanceFront1,long distanceFront2){
  
  bool obstacleDevant = (distanceFront1>0 && distanceFront1 < DISTANCE_SEUIL) || (distanceFront2>0 && distanceFront2 < DISTANCE_SEUIL);
  bool obstacleDerriere = (distanceRear>0 && distanceRear < DISTANCE_SEUIL_ARRIERE);
  ...
  switch(val){
        ...
        case 'z': 
          if (!obstacleDevant) {
            ...
            // on voit l'obstacle, on n'avance pas, mais on reste prêt
            reculer(100);
          }
          break;
        case 's': 
...
bool dist(long distanceFront1,long distanceFront2){
  return (distanceFront1 < DISTANCE_SEUIL && distanceFront1 != 0 || distanceFront2 < DISTANCE_SEUIL && distanceFront2 != 0);
}
