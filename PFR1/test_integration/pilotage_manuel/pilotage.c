...
void ajouter_mouvement(const char *mouvement, const char *description, int valeur) {
    ...
    FILE *fichier_csv = fopen("pilotage_manuel/Log_Deplacements.csv", "a");
    if (!fichier_csv) {
...
void pilotage_manuel() {
    ...
    FILE *fichier = fopen("pilotage_manuel/Log_Deplacements.csv", "w");
    char touche='a';
    while (touche!='x') {
...
void simulation_manuel () {
    const char *command = "python3 pilotage_manuel/simulateur.py";
